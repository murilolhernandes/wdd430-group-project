...
export default function CartClient({ products }: { products: Product[] }) {
  ...
  const populatedCart = cart.map(cartItem => {

    const productDetails = products.find(p => p.slug === cartItem.productId);
    return {
      ...cartItem,
        ...
        <div className="container-earth space-y-8">
          ...
          {populatedCart.length === 0 ? (
            <div className="earth-card p-12 text-center">
              ...
              <Link href="/shop" className="w-full bg-stone-800 !text-white px-6 py-4 rounded-md font-medium hover:bg-stone-700 transition">
                Return to Shop
              </Link>
            </div>
          ) : (
            <div className="grid gap-12 lg:grid-cols-3">
              ...
              <div className="lg:col-span-2 space-y-6">
                {populatedCart.map((item) => (
                  <div key={item.productId} className="earth-card p-6 flex flex-col sm:flex-row gap-6 items-center">
                    ...
                    <div className="flex-grow text-center sm:text-left">
                      <h3 className="text-xl font-semibold text-stone-800">{item.product?.name}</h3>
                      <p className="text-sm text-stone-500 mt-1">By {item.product?.artisan}</p>
                      <p className="mt-3 font-medium text-stone-700">Quantity: {item.quantity}</p>
                    </div>
              ...
              <div className="lg:col-span-1">
                <div className="earth-card p-8 sticky top-24">
                  <h3 className="text-xl font-semibold text-stone-800 mb-6">Order Summary</h3>
                  ...
                  <Link href="/checkout" className="block w-full text-center bg-stone-800 !text-white px-6 py-4 rounded-md font-medium hover:bg-stone-700 transition">
                    Proceed to Checkout
                  </Link>
                </div>
              </div>
